...
var maxInstances = require('../config').maxInstances || Infinity
if (isNaN(maxInstances)) {
  throw new Error('missing config option "maxInstances"')
}
...
Manager.prototype.start = function (server) {
  ...
  if (runningInstances + requiredInstances <= maxInstances) {
    server.start()
    ...
    return {
      ...
      'Currently, ' + runningInstances + ' are already running of the maximum number of ' + maxInstances + '.'
    }
  }
}
